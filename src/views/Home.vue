<template>
  <div class="interactive-bg" ref="bg">
    <div class="flex justify-between p-6">
      ...
      <button @click="router.push('/iniciar-sesion')"
        class="font-made-tommy border-rosy hover:bg-rosy hover:text-black border-[1px] text-rosy p-2 rounded-lg flex gap-2">
    ...
    <div class="h-full flex flex-col justify-center p-6 gap-12 items-center">
      <div class="font-made-tommy sm:w-2/3 w-full sm:h-[250px] flex flex-col gap-4 items-center sm:relative flex-none">
        <h1 class="sm:text-5xl text-4xl font-bold text-white text-start w-2/3">
          Con <span class="text-yellow">SKYDERS</span> prepárate para los eventos más emocionantes de México!
        </h1>
        <p class="text-lg text-white text-start sm:text-end sm:absolute sm:bottom-0 sm:w-1/3 w-2/3 sm:right-36">
          Encuentra boletos para todo tipo de eventos, en todo el país!
        </p>
      </div>
      <SearchBar />
    </div>
...
// Referencia al fondo interactivo
let bg;

onMounted(() => {
  // Cargar los eventos desde la API al montar el componente
  eventsStore.fetchEvents();

  // Configuración para el fondo interactivo
