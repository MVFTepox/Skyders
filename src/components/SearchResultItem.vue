<template>
    <div class="w-full flex items-center h-48 p-4 px-8 gap-6 sm:gap-12 border-b-[1px] border-white cursor-pointer font-made-tommy">
        ...
        <div class="flex flex-col text-lg md:text-xl  items-center text-white">
            <p>{{ props.month }}</p>
            <p class="font-bold text-xl sm:text-2xl">{{ props.day }}</p>
            <p>{{ props.year }}</p>
        </div>
        <div class="flex flex-col gap-4 text-sm  md:text-base">
            <p class="text-yellow">{{props.dayOfWeek}} • {{ props.time }} </p>
